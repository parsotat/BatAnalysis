...
FRB180916_dir="/Users/slaha/Desktop/BAT_projects/GIT_Repository_directory_Jan2022/FRB180916/"

list_of_obsid=np.genfromtxt("/Users/slaha/Desktop/BAT_projects/GIT_Repository_directory_Jan2022/FRB180916/list_of_obsid.txt", usecols=(0),unpack=True, dtype=str)
...
input_dict=dict(indir=None,outdir=None,incatalog="survey6b_2.cat")       #Creating a dictionary to set up the input and output directories and the files


for i in list_of_obsid[1:4]:

    print(i)
    try:
        
        obs=ba.BatSurvey(str(i),input_dict=input_dict,obs_dir=FRB180916_dir,recalc=True)
...
for i in list_of_obsid:
    ...
    try:
        batsurvey_obs.append(ba.BatSurvey(str(i), load_file=os.path.join(FRB180916_dir,"%s_surveyresult/batsurvey.pickle"%(str(i)))))
...
for obs in batsurvey_obs:
    ...
    obs.merge_pointings()
    obs.pointing_ids 
    obs.calculate_pha(id_list="FRB180916",clean_dir=True)
...
ba.print_parameters(batsurvey_obs,values=["met_time","utc_time", "exposure","flux","flux_lolim", "flux_hilim"],savetable=True, save_file=FRB180916_dir+"200_onwards_output.txt", latex_table=True)
