...
class BatSkyImage(Histogram):
    ...
    def __init__(
        ...
        if image_type is not None:
            ...
            if type(image_type) is not str or not np.any(
                [i == image_type for i in _accepted_image_types]
            ):
                raise TypeError(
    ...
    def _set_histogram(self, histogram_data=None, event_data=None, weights=None):
        ...
        if isinstance(histogram_data, u.Quantity) or isinstance(
        ...
        if not isinstance(histogram_data, Histogram):
            ...
            if np.ndim(histogram_data) != 4:
                raise ValueError(
                    f"The size of the input sky image is a {np.ndim(histogram_data)} dimensional array"
            ...
            if np.shape(histogram_data) != (
                ...
                histogram_data.shape[1],
                histogram_data.shape[2],
                self.ebins["E_MIN"].size,
            ):
                raise ValueError(
                    f"The shape of the input sky image is {np.shape(histogram_data)} while it should be"
    ...
    def healpix_projection(self, coordsys="galactic", nside=128):
        ...
        if "HPX" not in self.axes.labels:
            ...
            # coordinates
            new_array = np.zeros(
                (self.axes["TIME"].nbins, hp_ax.nbins, self.axes["ENERGY"].nbins)
            )
            ...
            for t in range(self.axes["TIME"].nbins):
                for e in range(self.axes["ENERGY"].nbins):
                    array, footprint = reproject_to_healpix(
                        (
                            self.slice[t, :, :, e].project("SKYY", "SKYX").contents,
                            self.wcs,
                        ),
                        coordsys,
                        nside=nside,
            ...
            h = BatSkyImage(
                image_data=Histogram(self.axes, contents=self.contents, unit=self.unit),
                image_type=self.image_type,
                is_mosaic_intermediate=self.is_mosaic_intermediate,
            )
        ...
        if projection is None:
            ...
            if "ra/dec" in projection.lower():
                ...
                cax = divider.append_axes(
                    "right", size="5%", pad=0.05, axes_class=maxes.Axes
                )
    ...
    def from_file(cls, file):
        ...
        if not input_file.exists():
            raise ValueError(
                f"The specified sky image file {input_file} does not seem to exist. "
        ...
        with fits.open(input_file) as f:
            for i in range(len(f)):
                ...
                if np.any(
                    [
                        name in header["EXTNAME"].lower()
                        for name in _file_extension_names
                    ]
                ):
        ...
        if n_times > 1:
            raise NotImplementedError(
                "The number of timebins for the sky images is greater than 1, which is "
        ...
        img_data = np.zeros(
            (n_times, img_headers[0]["NAXIS2"], img_headers[0]["NAXIS1"], n_energies)
        )
        ...
        imtype = [
            name
            for name in _file_extension_names
            if name in img_headers[0]["EXTNAME"].lower()
        ][0]
        if "image" in imtype:
            imtype = "flux"
