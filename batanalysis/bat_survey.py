...
class BatSurvey(BatObservation):
    ...
    def __init__(self, obs_id, obs_dir=None, input_dict=None, recalc=False, verbose=False, load_dir=None,
                 patt_noise_dir=None):
        """
        ...
        if recalc or not load_file.exists() or not complete_file.exists():
            ...
            if not self.obs_dir.joinpath("bat").joinpath("survey").is_dir() or not self.obs_dir.joinpath(
                    "auxil").is_dir():
                raise ValueError(
            ...
            obs_doy = \
            str(np.timedelta64((time - np.datetime64(time.astype("M8[Y]"), 'D')), 'D') + np.timedelta64(1, 'D')).split(
                " ")[0]
            ...
            if patt_noise_dir.is_dir():
                ...
                if len(sorted(patt_noise_dir.glob(f"*_{obs_year}{obs_doy}*"))) > 0:
                    ...
                    patt_dates = np.array(
                        [np.datetime64(datetime(int(i), 1, 1) + timedelta(int(j) - 1)) for i, j in zip(years, days)])
            ...
            if input_dict is None:
                input_dict_copy = dict(indir=str(self.obs_dir),
                                       outdir=str(self.obs_dir.parent / f"{self.obs_dir.name}_surveyresult"))

                input_dict_copy['incatalog'] = str(
                    dir.joinpath("data/survey6b_2.cat"))  # os.path.join(dir,'data/survey6b_2.cat')
                ...
                if 'outdir' not in input_dict_copy or input_dict_copy['outdir'] is None:
                    input_dict_copy['outdir'] = str(
                        self.obs_dir.parent / f"{self.obs_dir.name}_surveyresult")  # self.obs_dir + '_surveyresult'
                else:
                    # make this a fully resolved path
            ...
            self.pointing_flux_files = sorted(self.result_dir.glob(
                f"point*/*_{bs.params['ncleaniter']}.cat"))  # glob.glob(f"{self.result_dir}/point*/*_{bs.params['ncleaniter']}.cat")

            # need to extract the respective pointing IDs
            self.pointing_ids = []
            for pointing in self.pointing_flux_files:
                self.pointing_ids.append(
                    pointing.parent.name.split('_')[-1])  # os.path.split(pointing)[0].split('_')[-1])
            ...
            for pointing, id in zip(self.pointing_flux_files, self.pointing_ids):
                ...
                self.pointing_info[id] = dict(success=stat, fail_code=None, met_time=time_array,
                                              exposure=exposure_array, utc_time=utctime, mjd_time=mjdtime)
            ...
            if verbose:
                if len(self.pointing_flux_files) > 0:
                    print("There were %d pointings found for the obsid %s." % (
    ...
    def merge_pointings(self, input_dict=None, verbose=False):
        ...
        if input_dict is None or 'outfile' not in input_dict:
            output_dir = self.result_dir.joinpath(
                'merged_pointings_lc')  # os.path.join(self.result_dir, 'merged_pointings_lc')
        else:
        ...
        for i in self.pointing_flux_files:
            if input_dict is None:
                dictionary = dict(keycolumn="NAME", infile=str(i),
                                  outfile=str(output_dir.joinpath("%s.cat")))  # os.path.join(output_dir,"%s.cat"))
            else:
    ...
    def calculate_pha(self, id_list, output_dir=None, calc_upper_lim=False, bkg_nsigma=None, verbose=True,
                      clean_dir=False, single_pointing=None):
        """
        ...
        for id in id_list:
            ...
            catalog = merge_output_path.joinpath(
                f"{id}.cat")  # os.path.join(os.path.split(self.merge_input['outfile'])[0], id+".cat")
            try:
                ...
                for i in range(len(time_array)):
                    ...
                    if ((time_array[i] + tstart_sinceT0[i]) <= time_array[i] < (
                        ...
                        org_catfile_name = self.result_dir.joinpath(f"{pointing_array[i]}").joinpath(
                        ...
                        attfile = self.result_dir.joinpath(f"{pointing_array[i]}").joinpath(
                            f"{pointing_array[i]}.att")  # os.path.join(self.result_dir, pointing_array[i], pointing_array[i]+'.att')
                        dpifile = self.result_dir.joinpath(f"{pointing_array[i]}").joinpath(
                            f"{pointing_array[i]}_1.dpi")  # os.path.join(self.result_dir, pointing_array[i], pointing_array[i]+ '_1.dpi')
                        detmask = self.result_dir.joinpath(f"{pointing_array[i]}").joinpath(
                            f"{pointing_array[i]}.detmask")  # os.path.join(self.result_dir, pointing_array[i], pointing_array[i]+ '.detmask')
                        ...
                        input_dict = dict(outfile=str(output_srcmask), attitude=str(attfile), ra=str(raobj_array[i]), \
                                          dec=str(decobj_array[i]), infile=str(dpifile), detmask=str(detmask),
                                          clobber="YES")
                        ...
                        spec_col3 = fits.Column(name='STAT_ERR', format='E', unit='count/s',
            ...
            if pointing_file.exists():
                ...
                with  fits.open(str(pointing_file)) as file:
                    ...
                    for s in sources:
                        ...
                        idx = np.arange(len(pointing_file_sources))[self._compare_source_name(s,
                                                                                              pointing_file_sources)]  # np.where(s==pointing_file_sources)[0]
                        ...
                        if len(idx) != 0:
                            ...
                            if len(rate_array) == 8:
                                ...
                                rate_err_array = np.concatenate(
                                    (self.pointing_info[id][s]["rate_err"], [rate_err_2_tot]))
                                ...
                                snr_array = np.concatenate((self.pointing_info[id][s]["snr"], [snr_allband_num]))
    ...
    def get_pointing_info(self, pointing_id, source_id=None):
        ...
        if source_id is None:
            ...
            try:
                ...
                print(ke)
                raise ValueError(
                    "The dictionary for %s does not exist yet in the pointing id %s" % (source_id, pointing_id))
    ...
    def get_pha_filenames(self, id_list=None, pointing_id_list=None, getupperlim=False):
        ...
        if id_list is None:
            ...
            val = [i for i in self.pha_file_names_list if any(str(i) for j in id_list \
                                                              if self._compare_source_name(j,
                                                                                           str(i.name).split(split_str)[
                                                                                               0]))]
            if pointing_id_list is not None:
                # only get the pha filenames for the pointing ids specified
...
class MosaicBatSurvey(BatSurvey):
    ...
    def __init__(self, mosaic_dir, recalc=False, load_dir=None):
        ...
        if not load_file.exists() or recalc:
            ...
            for id in self.pointing_ids:
                file = fits.open(str(self.result_dir.joinpath(
                    'swiftbat_exposure_c0.img')))  # os.path.join(mosaic_dir, 'swiftbat_exposure_c0.img'))
                ...
                user_timebin = dict(met_time=tbin_start_met, met_stop_time=tbin_end_met, utc_time=tbin_start_utctime,
                                    mjd_time=tbin_start_mjdtime, utc_stop_time=tbin_end_utctime,
    ...
    def detect_sources(self, catalog_file=None, input_dict=None):
        ...
        tmp_all_src_file = self.result_dir.joinpath(
            'tmp_sources_tot.cat')  # os.path.join(self.result_dir, 'tmp_sources_tot.cat')
        ...
        hsp.ftcopy(infile=f'{tmp_all_src_file}[col *; DUP=(CATNUM == CATNUM{{-1}})?T:F;]', outfile=str(all_src_file),
                   clobber='YES')
        hsp.ftselect(
    ...
    def calculate_pha(self, id_list, output_dir=None, calc_upper_lim=False, bkg_nsigma=None, verbose=True,
        ...
        for id in id_list:
            ...
            try:
                ...
                spec_col2 = fits.Column(name='RATE', format='E', unit='count/s/pixel',
                                        array=scale * count_rate_array[0][:-1])
                spec_col3 = fits.Column(name='STAT_ERR', format='E', unit='count/s/pixel',
