...
### 2. Lower the mosaic partial-coding floor to 1%

`batanalysis.mosaic._pcodethresh` is set to `0.01` in this branch.

Why this matters:

- the installed package version validated in the downstream workflow already used a 1% mosaic floor
- leaving the source branch at `0.15` would keep excluding the same low-partial-coding pixels the rest of this fix is meant to preserve
- `BatSkyView` imports `_pcodethresh` from `mosaic.py`, so this constant controls both mosaic accumulation and the healpix detection defaults

### 3. Align low-partial-coding masking with the mosaic floor
...
### 4. Normalize mosaic pcode before applying detection thresholds
...
### 5. Use healpix-aware defaults in mosaic detection
...
- a source-branch mosaic floor still fixed at 15%
